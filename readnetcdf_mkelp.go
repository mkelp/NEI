...
func main() {
	...
	//open netcdf file
	ff, err := os.Open("/home/mkelp/work/src/NEI/spatialResults_mkelp_test_ptegu.ncf")
	if err != nil {
		panic(err)
	}
	...
	ff2, err := os.Open("/home/mkelp/work/src/NEI/index_numbers_test_ptegu.csv")
	if err != nil {
		panic(err)
	}
	r := csv.NewReader(ff2)
	...
	//new csv produced
	w, err := os.Create("mkelp_readin_netcdf_20161205.csv")
	if err != nil {
		panic(err)
	}
	...
	// Note: put back in concentration as the data object in this for loop, 12/08/16
	for scc, concentration := range concentrationholder {
		//
		//	fmt.Println("storedata hello channel3")
		// NOTE: for health impact calcs/maps 12/08/16
