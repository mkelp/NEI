...
func process(sector string, c *Config, fileTemplates []string, dataChan chan *dataHolder, wg *sync.WaitGroup) {
	...
	d.poltitle = []string{}
	//d.totalRows = int
	fmt.Println("process break", sector)
	...
	for _, rec := range data {
		...
		if _, ok := d.concentrationholder[scc]; !ok {
			...
			d.recordholder[scc] = rec
		} else {
			d.recordholder[scc].CombineEmissions(rec) // adds concentrations to the data holder
	...
	for _, x := range d.concentrationholder {
		for _, w := range x {
	...
	fmt.Println("process number2", len(d.concentrationholder), "sector", sector)
	dataChan <- d
...
func spatialOutputter(dataChan chan *dataHolder, wg *sync.WaitGroup) {
	...
	//creates netcdf file
	f, err := os.Create("/home/mkelp/work/src/NEI/spatialResults_mkelp_test_20161214.ncf")
	...
	m, err := os.Create("/home/mkelp/work/src/NEI/index_numbers_test_20161214.csv")
	if err != nil {
		panic(err)
	}
	mm := csv.NewWriter(m)
	...
	for sector, mk := range data {
		for scc, pollutants := range mk.concentrationholder {
			...
			for poptype, population := range c.pop {
				if popTypes != totalPop {
					continue
				}
